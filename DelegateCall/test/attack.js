...
describe("Attack", function () {
  it("Should change the owner of the Good contract", async function () {
    ...
    // Start the attack
    const [attackerAddress] = await ethers.getSigners()
    let tx = await _attackContract.attack(attackerAddress.address);
    await tx.wait();

    expect(await _goodContract.owner()).to.equal(attackerAddress.address);
  });
});
